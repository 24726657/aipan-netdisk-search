...
const paginationState = computed(() =>
  getPaginationState({
    totalItems: kwData.value.length,
    page: currentPage.value,
    pageSize: pageSize.value,
  }),
);

const paginatedKwData = computed(() =>
  buildPageItems(kwData.value, {
...
const visiblePageNumbers = computed(() => {
  const totalPages = paginationState.value.totalPages;
  const page = paginationState.value.page;
  const maxButtons = 5;
...
const changePage = async (page) => {
  ...
  const nextState = getPaginationState({
    totalItems: kwData.value.length,
    page,
    pageSize: pageSize.value,
  });
...
// flacdownloader returns a fixed result set, so pagination is client-side.
...
<template>
  <div class="bg-linear-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 min-h-screen">
    <div class="max-w-3xl mx-auto p-[20px]">
      ...
      <div class="flex flex-col items-center justify-center gap-3 mb-10">
        <div class="flex items-center gap-3 mb-2">
          <a href="/" class="transition-transform duration-200 hover:scale-110">
            <img class="w-[50px] h-[50px]" src="/logo-1.png" alt="logo" />
          </a>
          <h1
      ...
      <!-- Results Section -->
      <div id="music-results" class="space-y-4 scroll-mt-6" v-if="accessStatus.allowed && kwData && kwData.length > 0">
        <div class="flex flex-row gap-2 items-center justify-between p-2 border-b border-gray-200 dark:border-gray-700">
          ...
          <div class="hidden sm:flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            <span>第 {{ paginationState.start }}-{{ paginationState.end }} 首</span>
            <select
              class="rounded-lg border border-gray-200 bg-white px-2 py-1 text-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:focus:ring-indigo-900/40"
              :value="pageSize"
        ...
        <div v-else class="space-y-4">
          <ul class="space-y-2">
          <li v-for="item in paginatedKwData" :key="item.id"
            class="flex flex-row justify-between items-center p-4 bg-white rounded-xl hover:shadow-md transition-all duration-200 dark:bg-gray-800/70 dark:hover:bg-gray-800 dark:border dark:border-gray-700">
            <div class="flex items-center gap-3 flex-1 min-w-0">
              <!-- Album cover -->
              <div class="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-gray-200 dark:bg-gray-700">
          ...
          <div
            ...
            class="flex flex-col gap-3 rounded-xl border border-gray-200 bg-white p-3 shadow-sm dark:border-gray-700 dark:bg-gray-800/70"
          >
            <div class="flex items-center justify-between gap-3 text-sm text-gray-500 dark:text-gray-400">
              <span>
                第 {{ paginationState.page }} / {{ paginationState.totalPages }} 页 · 显示
                {{ paginationState.start }}-{{ paginationState.end }} 首
              </span>
              <select
                class="sm:hidden rounded-lg border border-gray-200 bg-white px-2 py-1 text-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 dark:focus:ring-indigo-900/40"
                :value="pageSize"
                :disabled="searchLoading"
                @change="changePageSize($event.target.value)"
              >
            ...
            <div class="flex flex-wrap items-center justify-center gap-2">
              <button
                class="rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-600 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 disabled:hover:border-gray-200 disabled:hover:bg-transparent disabled:hover:text-gray-600 dark:border-gray-700 dark:text-gray-300 dark:hover:border-indigo-800 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-300"
                :disabled="!paginationState.hasPrevious || searchLoading"
                @click="changePage(paginationState.page - 1)"
              >
                上一页
              </button>
              <button
                ...
                :class="page === paginationState.page
                  ? 'bg-indigo-600 text-white shadow-sm dark:bg-indigo-500'
                  : 'border border-gray-200 text-gray-600 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 dark:border-gray-700 dark:text-gray-300 dark:hover:border-indigo-800 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-300'"
                :disabled="searchLoading"
              ...
              <button
                class="rounded-lg border border-gray-200 px-3 py-2 text-sm font-medium text-gray-600 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600 disabled:hover:border-gray-200 disabled:hover:bg-transparent disabled:hover:text-gray-600 dark:border-gray-700 dark:text-gray-300 dark:hover:border-indigo-800 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-300"
                :disabled="!paginationState.hasNext || searchLoading"
                @click="changePage(paginationState.page + 1)"
              >
                下一页
              </button>
